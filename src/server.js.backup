...
function runFfmpeg(args, requestId = 'unknown') {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      if (code === 0) {
        console.log(`[FFMPEG-SUCCESS] ID: ${requestId} | Transcode completed successfully`);
        resolve({ ok: true });
      } else {
        console.log(`[FFMPEG-ERROR] ID: ${requestId} | Exit code: ${code} | Error: ${stderr.slice(-200)}`);
...
// Upload to Pinata
if (!PINATA_JWT) {
  console.log(`[TRANSCODE-ERROR] ID: ${requestId} | Error: PINATA_JWT not configured`);
  throw new Error('PINATA_JWT not configured on server');
}

console.log(`[IPFS-START] ID: ${requestId} | Starting IPFS upload to Pinata`);
...
console.log(`[TRANSCODE-META] ID: ${requestId} | Creator: ${creator} | Has thumbnail: ${!!thumbnail}`);
...
const outputSizeMB = fs.existsSync(outputPath) ? (fs.statSync(outputPath).size / 1024 / 1024).toFixed(2) : 'unknown';
const processingTime = ((Date.now() - new Date(timestamp).getTime()) / 1000).toFixed(1);
console.log(`[TRANSCODE-SUCCESS] ID: ${requestId} | CID: ${cid} | OutputSize: ${outputSizeMB}MB | ProcessingTime: ${processingTime}s | Creator: ${creator}`);

res.status(200).json({ cid, gatewayUrl });

  } catch (err) {
  console.log(`[TRANSCODE-ERROR] ID: ${requestId} | Error: ${err.message} | Stack: ${err.stack?.split('\n')[0]}`);
  res.status(500).json({ error: err.message || 'Transcode failed' });
} finally {
  // Cleanup
  console.log(`[TRANSCODE-CLEANUP] ID: ${requestId} | Cleaning up temporary files`);
